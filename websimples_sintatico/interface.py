...
class CompiladorApp:
    ...
    def criar_layout_principal(self):
        ...
        self.criar_botao_sidebar(sidebar, "Exemplo com Erros Léxicos", self.carregar_exemplo_erros)
        self.criar_botao_sidebar(sidebar, "Exemplo com Erros Sintáticos", self.carregar_exemplo_erros_sintaticos)
        ...
        btn_analisar_lexico = tk.Button(
            sidebar,
            text="Analisar Léxica",
        ...
        btn_analisar_lexico.pack(fill="x", pady=8)

        btn_analisar_sintatico = tk.Button(
            sidebar,
            text="Analisar Sintática",
            font=("Segoe UI", 11, "bold"),
            bg="#5e81ac",
            fg=BG_MAIN,
            activebackground="#7297d7",
            activeforeground=BG_MAIN,
            bd=0,
            cursor="hand2",
            command=self.executar_analise_sintatica,
            height=2
        )
        btn_analisar_sintatico.pack(fill="x", pady=8)
        ...
        lbl_resultados_title = tk.Label(
            painel_resultados, text="Tokens, Erros & Sintaxe", font=("Segoe UI", 11, "bold"),
    ...
    def carregar_exemplo_erros_sintaticos(self):
        codigo = (
            "# Exemplo com Diversos Erros Sintáticos para Teste\n"
            "pagina\n"
        ...
        self.editor.redraw_line_numbers()
        self.atualizar_posicao_cursor(1, 0)
        self.executar_analise_sintatica()
        self.compilacao_tempo_real()
    ...
    def executar_analise_sintatica(self):
        ...
        if total_erros_lexicos > 0:
            self.saida_text.insert(
                tk.END,
                "Não é possível executar a análise sintática porque existem erros léxicos. Corrija-os primeiro.\n",
                "erro"
            )
            self.atualizar_estatisticas(total_tokens, total_erros_lexicos)
            self.lbl_status_msg.config(
                text="ANÁLISE SINTÁTICA: interrompida devido a erros léxicos.",
                fg=COLOR_ERROR
            )
            self.saida_text.config(state="disabled")
            return
        
        ast, sintatic_errors, _ = analisar_sintatico(codigo)
        
        if sintatic_errors:
            for msg in sintatic_errors:
                self.saida_text.insert(tk.END, msg + "\n", "erro")
            self.lbl_status_msg.config(
                text=f"ANÁLISE SINTÁTICA: {len(sintatic_errors)} erro(s) detectado(s).",
                fg=COLOR_ERROR
            )
        else:
            # Gerar árvore sintática a partir do parser local
            tree_text = "Árvore Sintática gerada com sucesso."
            try:
                from sintatico import SintaticoParser
                parser_instance = SintaticoParser(codigo, tokens)
                ast = parser_instance.parse()
                tree_text = "Árvore Sintática:\n" + parser_instance.get_tree_text(ast)
            except Exception:
                tree_text = "Árvore Sintática gerada com sucesso."
            self.saida_text.insert(tk.END, "Análise sintática concluída com sucesso. Nenhum erro encontrado.\n", "token")
            self.saida_text.insert(tk.END, "\n" + tree_text + "\n", "token")
            self.lbl_status_msg.config(
                text="ANÁLISE SINTÁTICA: nenhum erro encontrado.",
                fg=COLOR_SUCCESS
            )
